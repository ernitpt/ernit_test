...
// ========== CREATE DEFERRED GIFT (TEST — ernitclone2) ==========
export const createDeferredGift_Test = onRequest(
    {
        region: "europe-west1",
    ...
    async (req, res) => {
        ...
        if (rateLimitSnap.exists) {
            ...
            if (requests.length >= RATE_LIMIT) {
                console.warn(`⚠️ [TEST] createDeferredGift rate limit exceeded for user ${userId}`);
        ...
        try {
            ...
            // Create Stripe SetupIntent (sandbox)
            const setupIntent = await stripe.setupIntents.create({
                metadata: {
                    giftId,
                    giverId: userId,
                    experienceId,
                    amount: String(Math.round((experienceData.price || 0) * 100)),
                    currency: 'eur',
                },
                usage: 'off_session',
            });
            ...
            console.log(`✅ [TEST] Created deferred gift ${giftId}, setupIntent ${setupIntent.id}`);
