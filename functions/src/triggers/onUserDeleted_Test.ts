// GDPR: Clean up all user data on account deletion (TEST — ernitclone2 database)
...
/**
 * Cloud Function: onUserDeleted_Test
...
export const onUserDeleted_Test = functionsV1.auth.user().onDelete(async (user: UserRecord) => {
    const uid = user.uid;
    logger.info(`[TEST] [onUserDeleted] Starting GDPR cleanup for uid=${uid}`);

    // Use getFirestore("ernitclone2") to target the test database
    ...
    try {
        ...
        logger.info(`[TEST] [onUserDeleted] Anonymized users/${uid}`);
        ...
        logger.info(`[TEST] [onUserDeleted] Soft-deleted goals for uid=${uid}`);
        ...
        logger.info(`[TEST] [onUserDeleted] Removed friendships for uid=${uid}`);
        ...
        logger.info(`[TEST] [onUserDeleted] Deleted notifications for uid=${uid}`);
        ...
        logger.info(`[TEST] [onUserDeleted] Deleted meta subcollection for uid=${uid}`);
        ...
        logger.info(`[TEST] [onUserDeleted] Deleted feed posts for uid=${uid}`);

        logger.info(`[TEST] [onUserDeleted] GDPR cleanup complete for uid=${uid}`);
        return null;
    } catch (error: unknown) {
        logger.error(`[TEST] [onUserDeleted] GDPR cleanup failed for uid=${uid}:`, error);
        return null;
    }
});
