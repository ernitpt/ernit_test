        ...
        try {
            ...
            for (const goalDoc of goalsSnap.docs) {
                ...
                try {
                    ...
                    console.error(
                        `⚠️ [TEST] Could not fetch experience name for goal ${goalDoc.id}:`,
                        expError
                    );
                }
                ...
                try {
                    ...
                    notificationsSent++;
                    console.log(
                        `✅ [TEST] Sent day-${daysSinceCompletion} booking reminder to user ${goal.userId} for goal ${goalDoc.id}`
                    );
                } catch (notifError) {
                    console.error(
                        `❌ [TEST] Failed to create booking reminder for goal ${goalDoc.id}:`,
                        notifError
                    );
                }
            }
