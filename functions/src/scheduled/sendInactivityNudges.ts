    ...
    async (event) => {
        try {
            ...
            while (true) {
                ...
                logger.info(`📊 [PROD] Processing page of ${goalsSnap.size} active goals (total so far: ${processed})`);
                ...
                for (const goalDoc of goalsSnap.docs) {
                    ...
                    if (isWeekCompleted) {
                        logger.info(
                            `⏭️ [PROD] Goal ${goalDoc.id}: Week completed, no sessions available yet — skipping nudge`
                        );
                        continue;
                    }
            ...
            logger.info(
                `✨ [PROD] Inactivity nudges check complete. Processed ${processed} goals. Sent ${notificationsSent} notification(s).`
            );
        } catch (error: unknown) {
