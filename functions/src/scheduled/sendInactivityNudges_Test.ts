...
export const sendInactivityNudges_Test = functions.onSchedule(
    ...
    async (event) => {
        try {
            console.log("🔍 [TEST] Starting inactivity nudges check...");
            ...
            console.log(
                `✨ [TEST] Inactivity nudges check complete. Sent ${notificationsSent} notification(s).`
            );
        } catch (error: unknown) {
            console.error("❌ [TEST] Error in sendInactivityNudges:", error);
        }
    }
);
