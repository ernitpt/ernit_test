...
const OnboardingScreen = () => {
    ...
    const onViewableItemsChanged = useRef(({ viewableItems }: { viewableItems: any[] }) => {
        if (viewableItems.length > 0) {
            ...
            logger.log('📍 Viewable items changed to index:', newIndex, 'slide:', slides[newIndex]?.title);
    ...
    const handleMomentumScrollEnd = (event: any) => {
        ...
        logger.log('🔄 Scroll ended at index:', newIndex, 'slide:', slides[newIndex]?.title);
        setCurrentIndex(newIndex);
    };
    ...
    const handleNext = () => {
        ...
        logger.log('   Current slide:', slides[currentIndex]?.title);
        logger.log('   Total slides:', slides.length);
        logger.log('   Is last slide?', currentIndex === slides.length - 1);

        if (currentIndex < slides.length - 1) {
            const nextIndex = currentIndex + 1;
            logger.log('   ✅ Advancing to index:', nextIndex, 'slide:', slides[nextIndex]?.title);
            ...
            logger.log('   ✅ Last slide - calling handleGetStarted');
            handleGetStarted();
        }
    };
