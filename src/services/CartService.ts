    ...
    try {
      if (Platform.OS === 'web' && typeof window !== 'undefined') {
        ...
        if (stored) {
          ...
          logger.log('[CartService] Loaded guest cart from localStorage:', parsed.length, 'items');
      ...
      if (stored) {
        const parsed = JSON.parse(stored);
        logger.log('[CartService] Loaded guest cart from AsyncStorage:', parsed.length, 'items');
        return Array.isArray(parsed) ? parsed : [];
      }
      return [];
    } catch (error) {
